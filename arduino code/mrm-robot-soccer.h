...
class RobotSoccer : public Robot {
  ...
  ActionSoccerlineDebug* actionlineDebug;
  ActionSoccerlineCal* actionlineCal;
...
public:
  ...
  void generateBinary(int num, int &a, int &b, int &c, int &d);
  ...
  void addNumberAtIndex(int value, int index, std::list<int>& myList);

  void removeNumberAtIndex(int index, std::list<int>& myList);

	bool lineAny();

	void lineAvoid();

  int get(const std::list<int>& myList, int index);
  ...
  void np();
  void nt();
  void odi(float speed, float direction_degrees, float rotation_speed);
...
*/
class ActionSoccerlineCal : public ActionBase {
	void perform() { ((RobotSoccer*)_robot)->lineCal(); }
public:
	ActionSoccerlineCal(RobotSoccer* robot) : ActionBase(robot, "clb", "Line Calibration", 8) {}
};

class ActionSoccerlineDebug : public ActionBase {
	void perform() { ((RobotSoccer*)_robot)->lineDebug(); }
public:
	ActionSoccerlineDebug(RobotSoccer* robot) : ActionBase(robot, "lin", "Line Debug", 8) {}
};
